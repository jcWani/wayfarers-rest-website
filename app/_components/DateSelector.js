...
function DateSelector({ settings, room, bookedDates }) {
  ...
  return (
    <div className="flex flex-col justify-between">
      ...
      <div className="flex items-center justify-between px-8 bg-borange-200 text-bistre-600 h-[72px]">
        <div className="flex items-baseline gap-6">
          <p className="flex gap-2 items-baseline">
            {discount > 0 ? (
              ...
              <span className="text2xl">₱{regularPrice}</span>
            )}
            <span className="">/night</span>
          </p>
          {numNights ? (
            <>
              <p className="bg-bistre-700 px-3 py-2 text-2xl text-bistre-200">
        ...
        {range?.from || range?.to ? (
          <button
            className="border border-bistre-500 py-2 px-4 text-sm font-semibold"
            onClick={() => resetRange()}
          >
            Clear
          </button>
